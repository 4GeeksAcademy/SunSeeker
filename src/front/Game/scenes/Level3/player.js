...
export function createColliders(game) {
  ...
  game.perritos.forEach((perrito) => {
    game.physics.add.overlap(game.GatoNar, perrito, () => {
      onHitEnemy(game);
    });
  });
}
...
function spawnPerrito(game, x, y, flip) {
  game.perrito = game.physics.add
    .sprite(x, y, "Perrito")
    .setOrigin(0, 0)
    .setScale(0.4)
    .setGravityY(-1000)
    .setVelocityX(flip ? 180 : -135);
  ...
  game.physics.add.overlap(game.perrito, game.zona, (p) => {
    p.setPosition(p.respawnX, p.respawnY).setVelocityX(flip ? 170 : -135);
  });
